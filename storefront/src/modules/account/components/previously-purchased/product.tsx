  ...
  return (
    <Container className="flex justify-between items-center">
      <div className="flex gap-2">
        <div className="w-14 h-14 rounded-md overflow-hidden bg-muted/30">
          <Thumbnail thumbnail={thumbnail} size="square" />
        </div>
        <div className="flex flex-col justify-center">
          <Text className="text-lg text-foreground">{product_title}</Text>
          <Text className="text-sm text-neutral-500">{title}</Text>
        </div>
      </div>
      <LocalizedClientLink href={`/products/${product_handle}`}>
        <Button variant="secondary" className="h-8 px-4 text-muted-foreground">
          Buy again
